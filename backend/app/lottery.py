...
# 诱饵金额：未中奖格随机展示的「差一点」奖金（真票上可见的小额奖级）
DECOY_AMOUNTS = (20, 20, 40, 40, 40, 60, 60, 60, 80, 80, 100, 100, 100, 200, 200, 500)

# 12 级真实赔率 (prize, one_in)：整体返还率≈65%、中奖率≈19.6%（与真票背面奖级表一致）。
...
def split_amount(amount: int, count: int, rng: random.Random) -> list[int]:
    ...
    金砖玩法专用；amount 必须是 10 的倍数且 ≥ count×10（REAL_ODDS 内 gold 资产档
    500/1000/5000 恒满足）。原版 game.js L164-176：全 10 起步、偏小分布抽增量、
