...
class UnitService:
    ...
    async def create_unit(self, data: dict) -> dict:
        ...
        if existing:
            return error(code=409, message=f"Unit sequence {data['sequence']} already exists")
        unit = Unit(**data)
    ...
    async def get_units(self, page: int = 1, page_size: int = 20) -> dict:
        units, total = await self.repo.get_all(page=page, page_size=page_size)
        items = []
        for u in units:
            wc = await self.repo.get_word_count(u.id)
            items.append(self._to_dict(u, word_count=wc))
